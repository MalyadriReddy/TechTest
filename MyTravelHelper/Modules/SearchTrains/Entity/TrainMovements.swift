...
//    }
    ...
    init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        trainCode = try values.decode(String.self, forKey: .trainCode)
        locationCode = try values.decode(String.self, forKey: .locationCode)
        locationFullName = try (values.decodeIfPresent(String.self, forKey: .locationFullName)) ?? "NA"
        expDeparture = try values.decode(String.self, forKey: .expDeparture)
    }
}
